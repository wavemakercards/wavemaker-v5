<template>
  <div class="documentStructure" v-if="this.$root.tools.writer.mybook">
    ...
    <div>
      <button class="docsettings" @click="$root.tools.writer.selectednode =  null">
        {{ this.$root.tools.writer.mybook.title }}
      </button>
      <DocStructureNode v-if="this.$root.tools.writer.mybook.files.length" v-model="this.$root.tools.writer.mybook.files" />
    </div>
...
export default {
  ...
  async mounted(){
   // this.mybook = await this.$root.useObservable(this.$root.liveQuery(async () => await this.$root.db.Writer.get(this.$root.tools.writer.selected)))
    this.bookwatch = await this.$root.useObservable(this.$root.liveQuery(async () => await this.$root.db.Writer.get(this.$root.tools.writer.selected)));
    ...
    async addNode(){
     ...
     await this.$root.AddRecord("Files", template);
      ...
      if(!this.$root.tools.writer.selectednode){
        ...
        addto = this.findNodeByUUID(this.$root.tools.writer.selectednode, this.$root.tools.writer.mybook)
      }

<script>
import MindmapEditor from '@/components/Apps/Mindmap/MindmapEditor.vue';
export default {
  name: "MindmapHome",
  components:{
    MindmapEditor
...
<template>
  <div v-if="!$root.tools.mindmap.selected" class="mindmaps">


      <button @click="$root.tools.mindmap.selected = item"  v-for="(item, i) in this.$root.syncdb.Mindmap">
      ...
      <button @click="$root.makeMindmap()" class="add">
  ...
  <div v-if="$root.tools.mindmap.selected">
    <MindmapEditor />
  </div>

  ...
  methods: {
    ...
    addTime(index) {
      ...
      if (index === undefined || index === null) {
        this.mysnowflake.content.items.push(obj);
      } else {
        this.mysnowflake.content.items.splice(index, 0, obj);
      }
    },
  },
};
...
<template>
  ...
  <div class="snowflake" v-if="mysnowflake">
    <ul>
      <li v-for="(item, index) in mysnowflake.content.items">
        <div class="content">
          <h3>
            <input type="text" v-model="item.title" placeholder="Title Here" @blur="update()" />
          </h3>
          <p><MiniEditor v-model="item.text" @blur="update()" :key="mysnowflake.lastupdated" /></p>
        </div>
        <div class="time">
          <h4>

<template>
    <VueDraggable :list="parentList" tag="transition-group" group="board" class="list-group-card" :component-data="{
      tag: 'div',
      type: 'transition-group',
      name: !drag ? 'flip-list' : null,
    }" handle=".cardHandle" v-bind="dragOptions" @start="drag = true" @end="drag = false" :emptyInsertThreshold="90"
  ...
  export default {
    ...
    methods: {
      linkCard(newuuid, uuid) {
        var element = this.parentList.find(x => x.uuid === uuid);
